...
const StaffViewReportPage = () => {
  const [reportList, setReportList] = useState([]); // State to hold the report files for the staff
  const [loading, setLoading] = useState(true); // State for loading indicator
  const location = useLocation(); // Get current route

  // Fetch the report files for the logged-in staff user
  useEffect(() => {
    const fetchStaffReportFiles = async () => {
      try {
        const response = await axios.get("https://mygolden.co.uk/staff-view-reports", {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`, // Include authorization token if needed
          },
        }); // Fetch report files for the logged-in staff
        setReportList(response.data); // Assuming response is an array of report files
        setLoading(false); // Stop loading after data is fetched
      } catch (error) {
        console.error("Error fetching staff report files:", error);
        setLoading(false); // Stop loading in case of error
      }
    };

    fetchStaffReportFiles();
  }, []);

  // Handle report file download
  const handleDownloadReport = (filePath) => {
    // Trigger file download
    window.location.href = `https://mygolden.co.uk/download-report/${filePath}`;
  };
      ...
      <div className="d-flex">
        ...
        <div className="sidebar bg-dark text-white p-4" style={{ width: "250px" }}>
          ...
          <ul className="nav flex-column">
            ...
            <li className="nav-item">
              <Link to="/staff-rota" className={`nav-link text-white ${isActive("/staff-view-rota")}`}>
        ...
        <div className="container mt-5 flex-grow-1">
          ...
          <p className="text-center">Here are the reports assigned to you.</p>
